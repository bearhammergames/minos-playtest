...
export const CURSES = [
  { id:'roll_lock', kind:'curse', label:'Roll-lock', desc:'One die locks at the start of each spin.', ctx:{ lockDice:1 } },
  { id:'grasping',  kind:'curse', label:'Grasping',  desc:'Keep at most 2 dice per spin.',           ctx:{ keepCap:2 } },
];
...
export function detectCombos(thread, extra = {}){
  ...
  for (const C of new Set(o.flatMap(colsOf))){
    let run = 0;
    for (const seg of o){ if (colsOf(seg).includes(C)){ run++; if (run>maxRun) maxRun=run; } else run = 0; }
  }
  ...
  push('double', topTier(COMBOS.double, o.filter(s=>(s.metCount||1)>=2).length));
...
export function tallyScore(thread, opts = {}){
  ...
  per.forEach(s => { const m = s.metCount || 1; if (m >= 2) multi += SCORE.multiRung * (m - 1); if (m >= 3) allThree += SCORE.allThree; });
